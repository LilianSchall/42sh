...
int exec_cond_2(void)
{
    ...
    struct AST *ast_true = new_AST(new_token(new_unique_symbols(copy_string("false"), false, false, false), (enum token_type) WORD), (enum AST_type) COMMAND, NULL);   
    ...
    struct AST *ast_cond = new_AST(new_token(new_unique_symbols(copy_string("||"), false, false, false), (enum token_type) OR), (enum AST_type) OPERATOR, ll_cond);
