...
int test14(void)  // ./scrip.sh 2> errorlog2.txt
{
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("errorlog2.txt"), (enum token_type) WORD), 
...
void redirection_14(void)
{
    cr_redirect_stdout();
    cr_redirect_stderr();
}

Test(exec_redirection, redirection_14, .init = redirection_14)
{
    test14();

    cr_assert_stdout_eq_str("in stdout\n");

    char *result_2 = get_file_content("errorlog2.txt");
