...
int exec_redirection_0(void)
{
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("testRedirect0.txt"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
...
void redirect_0(void)
{
    cr_redirect_stdout();
}

Test(exec_redirection, redirection_sup_0, .init = redirect_0)
{
    exec_redirection_0();
    fflush(stdout);

    char * result_2 = get_file_content("testRedirect0.txt");
