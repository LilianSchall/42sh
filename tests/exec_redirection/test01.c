...
Test(exec_redirection, redirection_1, .init = redirect_1)
{
    ...
    struct AST *ast_fd_to = new_AST(new_token(new_unique_symbols(copy_string("test11.txt"), false, false, false), (enum token_type) ARG), 
    (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_redirect = new_AST(new_token(new_unique_symbols(copy_string("1"), false, false, false), (enum token_type) R_SUP), 
