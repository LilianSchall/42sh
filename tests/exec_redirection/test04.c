...
{
    // echo hello > file.txt
    struct AST *ast_hello = new_AST(new_token(copy_string("foo"), (enum token_type) ARG, false), (enum AST_type) ARG, NULL);   
    ...
    struct AST *ast_fd_to = new_AST(new_token(copy_string("2"), (enum token_type) IO_NUMBER, false), 
    (enum AST_type) ARG, NULL);
