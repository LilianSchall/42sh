...
Test(exec_redirection, redirection_5, .init = redirect_5)
{
    ...
    struct AST *ast_hello = new_AST(new_token(copy_string("boo"), (enum token_type) ARG), (enum AST_type) ARG, NULL);   
    ...
    struct AST *ast_fd_to = new_AST(new_token(copy_string("test51.txt"), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
