...
Test(exec_redirection, redirection_5, .init = redirect_5)
{
 ...
 struct AST *ast_hello = new_AST(new_token(new_unique_symbols(copy_string("ezzzz"), false), (enum token_type) ARG), (enum AST_type) ARG, NULL);   

    struct linked_list *ll_ast = new_list();
    ll_ast = list_append(ll_ast, ast_hello);
    
    struct AST *ast_echo = new_AST(new_token(new_unique_symbols(copy_string("ls"), false), (enum token_type) ARG), (enum AST_type) COMMAND, ll_ast);
    ...
    struct AST *ast_fd_from = new_AST(new_token(new_unique_symbols(copy_string("2"), false), (enum token_type) IO_NUMBER), 
    (enum AST_type) ARG, NULL);

    struct AST *ast_fd_to = new_AST(new_token(new_unique_symbols(copy_string("test51.txt"), false), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
