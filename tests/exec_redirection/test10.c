...
Test(exec_redirection, redirection_if_10, .init = redirect_10)
{
    ...
    struct AST *ast_bloc = new_AST(new_token(copy_string(""), (enum token_type) WORD), (enum AST_type) SEQUENCE, ll_ast2);
    ...
    struct AST *ast_cond = new_AST(new_token(copy_string(""), (enum token_type) WORD), (enum AST_type) SEQUENCE, ll_ast_cond);
    ...
    struct AST *ast_if = new_AST(new_token(copy_string("if"), (enum token_type) IF), 
    ...
    struct AST *ast_fd_to = new_AST(new_token(copy_string("test101.txt"), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
