...
int test13(void)  // ./scrip.sh 2>> errorlog.txt
{
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("errorlog.txt"), (enum token_type) WORD), 
...
void redirection_13(void)
{
    cr_redirect_stdout();
    cr_redirect_stderr();
}

Test(exec_redirection, redirection_13, .init = redirection_13)
{
    test13();

    cr_assert_stdout_eq_str("in stdout\n");

    char *result_2 = get_file_content("errorlog.txt");
