...
int exec_redirection_6(void)
{
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("stdout.txt"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
...
void redirect_sup_2(void)
{
    cr_redirect_stdout();
}

Test(exec_redirection, redirection_sup_and_2, .init = redirect_sup_2)
{
    exec_redirection_6();
    fflush(stdout);

    char * result_2 = get_file_content("stdout.txt");
