...
Test(exec_redirection, redirection_6, .init = redirect_6)
{
    ...
    struct AST *ast_fd_from = new_AST(new_token(copy_string("0"), (enum token_type) IO_NUMBER), 
    (enum AST_type) ARG, NULL);

    struct AST *ast_fd_to = new_AST(new_token(copy_string("README.md"), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_redirect = new_AST(new_token(copy_string("<>"), (enum token_type) R_INF_SUP), 
