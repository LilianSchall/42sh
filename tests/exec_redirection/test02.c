...
Test(exec_redirection, redirection_2, .init = redirect_2)
{
    ...
    struct AST *ast_hello = new_AST(new_token(copy_string("hello"), (enum token_type) ARG, false), (enum AST_type) ARG, NULL);   
    ...
    struct AST *ast_fd_to = new_AST(new_token(copy_string("test22.txt"), (enum token_type) WORD, false), 
    (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_fd_from2 = new_AST(new_token(copy_string("1"), (enum token_type) IO_NUMBER, false), 
    (enum AST_type) ARG, NULL);

    struct AST *ast_fd_to2 = new_AST(new_token(copy_string("test21.txt"), (enum token_type) WORD, false), 
    (enum AST_type) ARG, NULL);
