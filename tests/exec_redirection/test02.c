...
Test(exec_redirection, redirection_2, .init = redirect_2)
{
    ...
    struct AST *ast_fd_to = new_AST(new_token(new_unique_symbols(copy_string("test22.txt"), false, false, false), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_fd_from2 = new_AST(new_token(new_unique_symbols(copy_string("1"), false, false, false), (enum token_type) IO_NUMBER), 
    (enum AST_type) ARG, NULL);

    struct AST *ast_fd_to2 = new_AST(new_token(new_unique_symbols(copy_string("test21.txt"), false, false, false), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
