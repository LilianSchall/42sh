...
Test(exec_redirection, redirection_7, .init = redirect_7)
{
    ...
    struct AST *ast_fd_to = new_AST(new_token(new_unique_symbols(copy_string("README.md"), false), (enum token_type) WORD), 
    (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_redirect = new_AST(new_token(new_unique_symbols(copy_string("<"), false), (enum token_type) R_INF), 
