...
Test(exec_redirection, redirection_17, .init = redirect_17)
{
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("test17.txt"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_file2 = new_AST(new_token(copy_string("1"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_redirect2 = new_AST(new_token(copy_string("2>&"), (enum token_type) WORD), (enum AST_type) REDIRECTION, ll_seq3);
    ...
    char *result_3 = get_file_content("test17.txt");
