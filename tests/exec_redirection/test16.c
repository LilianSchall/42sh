...
Test(exec_redirection, redirection_16, .init = redirect_16)
{
   // ./scipt.sh 1> Dvalid.txt 2> Derror.txt
    ...
    struct AST *ast_file = new_AST(new_token(copy_string("1"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
    ...
    struct AST *ast_file2 = new_AST(new_token(copy_string("test16.txt"), (enum token_type) WORD), (enum AST_type) ARG, NULL);
    ...
    char *result_3 = get_file_content("test16.txt");
