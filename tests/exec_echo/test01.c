...
int exec(void)
{
    struct AST *ast_1 = new_AST(new_token(new_unique_symbols(copy_string("hello"), false, false, false), (enum token_type) WORD), (enum AST_type) ARG, NULL);   
    struct AST *ast_2 = new_AST(new_token(new_unique_symbols(copy_string("!"), false, false, false), (enum token_type) WORD), (enum AST_type) ARG, NULL);
