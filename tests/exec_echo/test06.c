...
int exec6(void)
{
    struct AST *ast_1 = new_AST(new_token(new_unique_symbols(copy_string("README.md"), false), (enum token_type) WORD), (enum AST_type) ARG, NULL);   
    struct AST *ast_2 = new_AST(new_token(new_unique_symbols(copy_string("-e"), false), (enum token_type) WORD), (enum AST_type) ARG, NULL);
    
    struct linked_list *ll_ast = new_list();
    ll_ast = list_append(ll_ast, ast_1);
    ll_ast = list_append(ll_ast, ast_2);
    
    struct AST *ast = new_AST(new_token(new_unique_symbols(copy_string("cat"), false), (enum token_type) WORD), (enum AST_type) COMMAND, ll_ast);
