...
Test(exec_redirection, pipe_1, .init = redirect_pipe_1)
{
   ...
   // echo hello 
    struct AST *ast_c = new_AST(new_token(new_unique_symbols(copy_string("-c"), false), (enum token_type) ARG), (enum AST_type) ARG, NULL);   
    struct AST *ast_5 = new_AST(new_token(new_unique_symbols(copy_string("10"), false), (enum token_type) ARG), (enum AST_type) ARG, NULL);   
