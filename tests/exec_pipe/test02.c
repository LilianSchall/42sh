...
Test(exec_redirection, pipe_2, .init = redirect_pipe_2)
{
    ...
    struct AST *ast_seq_echo = new_AST(new_token(new_unique_symbols(copy_string(""), false, false, false), (enum token_type) WORD), 
                (enum AST_type) SEQUENCE, ll_command);


   // echo hello 
    struct AST *ast_c = new_AST(new_token(new_unique_symbols(copy_string("-c"), false, false, false), (enum token_type) WORD), (enum AST_type) ARG, NULL);   
    struct AST *ast_5 = new_AST(new_token(new_unique_symbols(copy_string("1"), false, false, false), (enum token_type) WORD), (enum AST_type) ARG, NULL);   
    ...
    // cat -e 
    struct AST *ast_e = new_AST(new_token(new_unique_symbols(copy_string("-e"), false, false, false), (enum token_type) WORD), 
    ...
    struct AST *ast_seq_cat = new_AST(new_token(new_unique_symbols(copy_string(""), false, false, false), (enum token_type) WORD), 
