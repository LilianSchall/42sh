...
int exec_while_1(void)
{
    ...
    struct AST *ast_while = new_AST(new_token(copy_string("while"), (enum token_type) WHILE, false), (enum AST_type) CONDITION, ll_while);

    struct linked_list *ll_seq = new_list();
    ll_seq = list_append(ll_seq, ast_while);

    struct AST *ast_seq = new_AST(new_token(copy_string(""), (enum token_type) COMMAND, false), (enum AST_type) SEQUENCE, ll_seq);
