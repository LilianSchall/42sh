...
static struct AST *and_or_rule(struct linked_list *token_list,
                               bool trigger_warn);
static struct AST *pipeline_rule(struct linked_list *token_list,
                                 bool trigger_warn);
static struct AST *command_rule(struct linked_list *token_list,
                                bool trigger_warn);
static struct AST *simple_command_rule(struct linked_list *token_list,
                                       bool trigger_warn);
static struct AST *element_rule(struct linked_list *token_list,
                                bool trigger_warn);
static struct AST *shell_command_rule(struct linked_list *token_list,
                                      bool trigger_warn);
static struct AST *rule_if_rule(struct linked_list *token_list,
                                bool trigger_warn);
static struct AST *else_clause_rule(struct linked_list *token_list,
                                    bool trigger_warn);
static struct AST *compound_list_rule(struct linked_list *token_list,
                                      bool trigger_warn);
...
struct AST *else_clause_rule(struct linked_list *token_list, bool trigger_warn)
{
    ...
    struct AST *tree =
        new_AST(new_token(strdup("else"), ELSE), SEQUENCE, new_list());
    struct AST *if_tree =
        new_AST(new_token(strdup("if"), IF), CONDITION, new_list());
