...
struct AST *else_clause_rule(struct linked_list *token_list, bool trigger_warn)
{
    ...
    struct AST *tree =
        new_AST(new_token(new_unique_symbols(strdup("else"),false, false, false), ELSE), SEQUENCE, new_list());
    struct AST *if_tree =
        new_AST(new_token(new_unique_symbols(strdup("if"), false, false, false), IF), CONDITION, new_list());
