...
static struct token *get_standard_io_number(enum token_type type)
{
    if (type == R_SUP || type == R_SUP_SUP || type == R_SUP_AND
        || type == R_SUP_PIPE)
        return new_token(new_unique_symbols(gc_strdup("1"), false, false, false), IO_NUMBER);
    // else
    return new_token(new_unique_symbols(gc_strdup("0"), false, false, false), IO_NUMBER);
}
